...
export function usePromiseInterval<T>(props: IUsePromiseInterval.Props<T>) {
  const { intervalTime, isCallWhenStarted, isForceCallWhenFnExecuting, isAutoStart, callMaxCount, fn } = props;
  ...
  const callMaxCountRef = useRef<number | undefined>(undefined);
