...
export function useBabylonCharacterController(props: IUseBabylonCharacterController.Props) {
  ...
  const firstQuaternionWithCameraRef = useRef<Quaternion | undefined>(undefined);
  ...
  async function add(
    params: IUseBabylonCharacterController.AddRequireInfo
  ): Promise<undefined | IUseBabylonCharacterController.CharacterItem> {
    const {
      camera,
      characterId,
  ...
  function setCharacterMoving(options: IUseBabylonCharacterController.CharacterMovingOptions) {
    const { characterId, direction, cameraDirection, isRunning } = options;
  ...
  function checkNearUser() {
    ...
    const target = arr.find(([characterId, characterItem]) => {
      ...
      if (
        distance < (thisClientCharacterOptions?.nearDistance ?? 2) &&
        (characterItem.direction !== undefined || characterItem.jumpingInterval !== undefined)
      ) {
        return true;
      }
  ...
  useRequestAnimationFrameManager({
    ...
    callback(startedTimestamp, currentTimestamp, step) {
      charactersRef.current.forEach((characterItem, characterId) => {
        ...
        const walkingAnim = characterItem.characterAnimationGroups.get(
          characterItem.characterAnimationGroupNames?.walkingAnimationGroupName ?? ''
        );
        const idleAnim = characterItem.characterAnimationGroups.get(
          characterItem.characterAnimationGroupNames?.idleAnimationGroupName ?? ''
        );
        const jumpAnim = characterItem.characterAnimationGroups.get(
          characterItem.characterAnimationGroupNames?.jumpingAnimationGroupName ?? ''
        );
        const runningAnim = characterItem.characterAnimationGroups.get(
          characterItem.characterAnimationGroupNames?.runningAnimationGroupName ?? ''
        );
        ...
        if (keydown) {
          ...
          if (characterItem.characterBox.rotationQuaternion !== null) {
            characterItem.characterMeshes.forEach((o) => {
              Quaternion.SlerpToRef(o.rotationQuaternion!, quaternion, 0.1, o.rotationQuaternion!);
            });
          }
