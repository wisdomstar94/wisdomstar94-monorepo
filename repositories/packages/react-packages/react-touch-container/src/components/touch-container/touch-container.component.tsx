...
export function TouchContainer(props: ITouchContainer.Props) {
  ...
  const touchTypeRef = useRef<ITouchContainer.PressType | undefined>(undefined);
  ...
  function onPress(event: MouseEvent | TouchEvent) {
    ...
    const isContain =
      bounds.x <= clientX && bounds.x + bounds.width >= clientX && bounds.y <= clientY && bounds.y + bounds.height >= clientY;
    if (!isContain) return;
