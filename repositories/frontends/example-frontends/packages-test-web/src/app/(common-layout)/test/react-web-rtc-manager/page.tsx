        ...
        case 'failed':
          ...
          if (peerConnectionInfo.type === 'sendOffer') {
            socketioManager.emit({
              ...
              data: {
                targetClientId: clientId !== peerConnectionInfo.clientId ? peerConnectionInfo.clientId : peerConnectionInfo.receiveId,
              },
            });
          }
          break;
  ...
  useEffect(() => {
    ...
    socketioManager.connect({
      socketUrl:
        ...
        (() => {
          throw new Error(`NEXT_PUBLIC_WEBS_BABYLON_JS_EXAMPLE_SOCKET_CONNECT_URL is not defined!`);
        })(),
      opts: {
        auth: {
          clientId,
    ...
    <>
      <div className="w-full flex flex-wrap gap-2 relative">
        <table className="w-full relative text-xs border-t border-l border-slate-400">
          <tbody>
            <tr>
              <th className="border-r border-b border-slate-400 text-left bg-slate-200">현재 내 clientId</th>
              <td className="border-r border-b border-slate-400 p-1">
                <div className="px-1 py-0.5 bg-blue-100 border border-slate-600 font-bold text-purple-500">{clientId}</div>
              </td>
            </tr>
            <tr>
              <th className="border-r border-b border-slate-400 text-left bg-slate-200">web rtc 로 peer 연결된 정보</th>
                    ...
                    return (
                      <li key={key} className="w-full flex flex-wrap p-1">
                        <div className="px-1 py-0.5 bg-blue-100 relative w-full flex flex-wrap gap-1 border border-slate-600">
                          ...
                          <div className="w-full font-bold text-purple-500">{clientId}</div>
                          <div className="w-full h-[1px] bg-black"></div>
                          <div className="w-full">{receiveId}</div>
                        </div>
                      </li>
                    );
                  })}
        ...
        <div className="w-full flex flex-wrap gap-2 relative text-xs items-end">
          <div className="inline-flex flex-col">
            <label>data channel name</label>
            <input
              type="text"
              className="border border-slate-500"
              value={inputedDataChannelName}
              onChange={(e) => setInputedDataChannelName(e.target.value)}
            />
          </div>
          <div className="inline-flex flex-col">
            <label>data</label>
          ...
          {Array.from(webRtcManager.getPeerConnectionDataChannelMap()).map(([key, value]) => {
            return (
              <div key={key} className="w-full border border-red-400 px-2 py-1 text-xs">
                key : {key}, channel : {value.channel.label}
              </div>
            );
          })}
        </div>
