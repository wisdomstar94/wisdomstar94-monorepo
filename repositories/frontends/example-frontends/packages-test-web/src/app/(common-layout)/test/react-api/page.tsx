...
export default function Page() {
  ...
  useEffect(() => {
    if (api.result === undefined) return;
      ...
      <div className="w-full relative flex flex-wrap gap-2">
        <div className="w-full relative flex flex-wrap gap-2">
          <div className="w-full relative flex flex-wrap gap-2">
            <button
              onClick={() => api.fetch()}
              className="inline-flex px-3 py-1.5 text-xs text-slate-600 border border-slate-400 rounded-lg cursor-pointer hover:bg-slate-100"
            >
              api fetch 하기
            </button>
          </div>
        ...
        <div className="w-full relative flex flex-wrap gap-2">{api.isLoading ? '로딩중...' : ''}</div>
