...
export default function Page() {
  ...
  useRequestAnimationFrameManager({
    ...
    callback: () => {
      ...
      if (cube.rotationQuaternion === null) {
        cube.rotationQuaternion = Quaternion.Identity();
      }

      if (keyDownStates.current.ArrowUp && keyDownStates.current.ArrowLeft) {
        cubeAggregate.body.setTargetTransform(
          new Vector3(cube.absolutePosition.x - 0.01, cube.absolutePosition.y, cube.absolutePosition.z - 0.01),
          cube.rotationQuaternion
        );
      } else if (keyDownStates.current.ArrowUp && keyDownStates.current.ArrowRight) {
        cubeAggregate.body.setTargetTransform(
          new Vector3(cube.absolutePosition.x - 0.01, cube.absolutePosition.y, cube.absolutePosition.z + 0.01),
          cube.rotationQuaternion
        );
      } else if (keyDownStates.current.ArrowDown && keyDownStates.current.ArrowLeft) {
        cubeAggregate.body.setTargetTransform(
          new Vector3(cube.absolutePosition.x + 0.01, cube.absolutePosition.y, cube.absolutePosition.z - 0.01),
          cube.rotationQuaternion
        );
      } else if (keyDownStates.current.ArrowDown && keyDownStates.current.ArrowRight) {
        cubeAggregate.body.setTargetTransform(
          new Vector3(cube.absolutePosition.x + 0.01, cube.absolutePosition.y, cube.absolutePosition.z + 0.01),
          cube.rotationQuaternion
        );
      } else if (keyDownStates.current.ArrowUp) {
        cubeAggregate.body.setTargetTransform(
          new Vector3(cube.absolutePosition.x - 0.01, cube.absolutePosition.y, cube.absolutePosition.z),
          cube.rotationQuaternion
        );
      } else if (keyDownStates.current.ArrowDown) {
        cubeAggregate.body.setTargetTransform(
          new Vector3(cube.absolutePosition.x + 0.01, cube.absolutePosition.y, cube.absolutePosition.z),
          cube.rotationQuaternion
        );
      } else if (keyDownStates.current.ArrowLeft) {
        cubeAggregate.body.setTargetTransform(
          new Vector3(cube.absolutePosition.x, cube.absolutePosition.y, cube.absolutePosition.z - 0.01),
          cube.rotationQuaternion
        );
      } else if (keyDownStates.current.ArrowRight) {
        cubeAggregate.body.setTargetTransform(
          new Vector3(cube.absolutePosition.x, cube.absolutePosition.y, cube.absolutePosition.z + 0.01),
          cube.rotationQuaternion
        );
