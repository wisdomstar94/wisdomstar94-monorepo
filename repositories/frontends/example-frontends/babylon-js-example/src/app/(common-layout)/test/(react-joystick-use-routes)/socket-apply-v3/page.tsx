...
export default function Page() {
  ...
  const shadowGeneratorRef = useRef<ShadowGenerator | undefined>(undefined);
  ...
  const babylonCharacterController = useBabylonCharacterController({
    ...
    onAdded(characterItem, scene) {
      ...
      const nicknameBgBox = MeshBuilder.CreateBox(
        'nickname-bg-box',
        {
      ...
      const nicknameTextPlane = MeshBuilder.CreatePlane(
        'nickname-text-plane',
        {
          width: nicknameBoxSize.width, // x 축 길이
    ...
    if (meCharacter !== undefined) {
      ...
      const data: IUseBabylonCharacterController.AddRequireInfoWithoutScene = {
        ...
        characterInitPosition: {
          x: meCharacter.characterBox.position.x,
          y: meCharacter.characterBox.position.y,
          z: meCharacter.characterBox.position.z,
        },
  ...
  const webRtcManager = useWebRtcManager<MetaData>({
    defaultRtcConfiguration: {
      ...
      iceServers: [
        {
          urls:
            process.env.NEXT_PUBLIC_ICE_SERVER_URL ??
            (() => {
              throw new Error(`NEXT_PUBLIC_ICE_SERVER_URL is not defined!`);
            })(),
          username:
            process.env.NEXT_PUBLIC_ICE_SERVER_USERNAME ??
            (() => {
              throw new Error(`NEXT_PUBLIC_ICE_SERVER_USERNAME is not defined!`);
            })(),
          credential:
            process.env.NEXT_PUBLIC_ICE_SERVER_CREDENTIAL ??
            (() => {
              throw new Error(`NEXT_PUBLIC_ICE_SERVER_CREDENTIAL is not defined!`);
            })(),
        },
      ],
    },
    ...
    onCreatedPeerConnectionInfo(peerConnectionInfo) {
      const { clientId, receiveId, rtcPeerConnection, meta } = peerConnectionInfo;
    ...
    onConnectionStateChange(peerConnectionInfo, event) {
      switch (peerConnectionInfo.rtcPeerConnection.connectionState) {
        ...
        case 'disconnected':
          break;
        case 'failed':
          babylonCharacterController.remove(
            peerConnectionInfo.clientId !== characterId ? peerConnectionInfo.clientId : peerConnectionInfo.receiveId
          );
  ...
  function disposeMoving(params: {
    ...
    const movingInfo: IUseBabylonCharacterController.CharacterMovingOptions = {
      characterId,
      direction,
      isRunning: isRunning,
  ...
  async function onReady(initInfo: IBabylonCanvas.InitInfo) {
    const { engines, scene, canvas, axesViewer } = initInfo;
    ...
    babylonMeshPhysicsManager.injectObject({
      ...
      physicsBody: (params) => {
        const { mesh } = params;
        const body = new PhysicsBody(mesh, PhysicsMotionType.STATIC, false, scene);
        body.shape = new PhysicsShapeBox(new Vector3(0, 0, 0), Quaternion.Identity(), new Vector3(0.5, 5, 40), scene);
        body.setMassProperties({ mass: 0 });
        return body;
      },
    });
  ...
  useEffect(() => {
    if (
      ...
      babylonCharacterController.isExistThisClientCharacterNearOtherCharacters
    ) {
      emitMeCurrentPositionAndRotationInterval.stop();
      emitMeCurrentPositionAndRotationInterval.start({
        intervalTime: babylonCharacterController.isThisClientCharacterControlling ? 50 : 1000,
      });
